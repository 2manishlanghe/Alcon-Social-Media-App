...
export const editPost = (data) => async (dispatch) => {
  ...
  try {
    const res = await fetch("http://localhost:5454/api/posts/edit", {
      method: "POST",
...
export const findUserPost = (data) => async (dispatch) => {
  ...
  try {
    const res = await fetch(`http://localhost:5454/api/posts/`, {
      method: "GET",
...
export const searchHashtagPostAction = (data) => async (dispatch) => {
  try {
    console.log("searchHashtagActio method :" + data.hashtag);
    const res = await fetch(
      `http://localhost:5454/api/posts/search/hashtag?h=${data.hashtag}`,
      {
        method: "GET",
    ...
    console.log("---Posts in Hashatg" + users);

    console.log("--- find by Hashatg posts  --- ", users);

    dispatch({ type: SEARCH_HASHTAG_POST, payload: users });
  } catch (error) {
    console.log("catch error -  ", error);
  }
};
