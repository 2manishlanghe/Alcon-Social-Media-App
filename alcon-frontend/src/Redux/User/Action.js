...
export const removeProfilePicture = (data) => async (dispatch) => {
  try {
    const res = await fetch(
      `http://localhost:5454/api/users/profile/remove-picture/${data?.data?.id}`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    dispatch({ type: REMOVE_PROFILE_PICTURE, payload: data?.data?.id });
  } catch (error) {
    console.log("Error occurred while removing profile picture: ", error);
  }
};
